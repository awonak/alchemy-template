...
int main()
{
    ...
    /* presets.Init() and presets.BootLoad() must be called AFTER loop.Use(...)
     * so that HostLink can introspect the attached Pages from the ControlLoop
     * and generate the JSON descriptor. */
    presets.Init();
    presets.BootLoad();
